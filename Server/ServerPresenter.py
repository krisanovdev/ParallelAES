...
class ServerPresenter:
    ...
    def __init__(self, filename):
        ...
        while position != data_length:
            right_bound = position + min(data_length - position, ServerPresenter.BLOCK_SIZE)
        ...
        os.makedirs(ServerPresenter.DECRYPT_FOLDER, exist_ok=True)
    ...
    def __on_part_of_task_done(self, task_index, data):
        try:
            if not data:
                return
            chunk_path = os.path.join(ServerPresenter.DECRYPT_FOLDER, 'chunk' + str(task_index))
    ...
    def __merge_chunks(self):
        result_file_path = os.path.join(ServerPresenter.DECRYPT_FOLDER, self.filename)
        os.rename(os.path.join(ServerPresenter.DECRYPT_FOLDER, 'chunk0'), result_file_path)
        result_file = open(result_file_path, 'ab')
        for i in range(1, self.tasks_count):
            chunk_path = os.path.join(ServerPresenter.DECRYPT_FOLDER, 'chunk' + str(i))
